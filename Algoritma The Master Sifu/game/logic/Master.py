...
class BoardUtilities:
    """Kelas untuk fungsi-fungsi utility yang berkaitan dengan board game"""
    
    @staticmethod
    def get_teleports(board: Board) -> list[Position]:
        """Mengambil semua posisi teleport dari board"""
        teleport_gate = list()
        obj: GameObject
    ...
    def get_reset(board: Board) -> List[GameObject]:
        """Mengambil semua diamond button (reset button) dari board"""
        reset = list()
        obj: GameObject
        for obj in board.game_objects:
...
class PathfindingUtils:
    """Kelas untuk kalkulasi pathfinding dan movement"""
    
    @staticmethod
    def get_direction(curr: Position, dest: Position, teleports: List[Position], allowed_directions) -> Tuple[int, int]:
        """Menentukan arah gerak terbaik menuju destinasi dengan mempertimbangkan teleport"""
    ...
    def distance(obj1: Position, obj2: Position, teleports: List[Position]):
        """Menghitung jarak minimum antara dua posisi dengan mempertimbangkan teleport"""
        pos_x = obj1.x
        pos_y = obj1.y
...
class master(BaseLogic):
    """
    Master logic class untuk bot game
    ...
    Jadi instead of finding nearest diamond, kita cari diamond terbanyak dalam jangkauan 15 dari jarak kita sekarang, 
    apabila diamond tersebut lebih dekat ke lawan, maka poin diamond tersebut effectively 0.7 dari worthnya.
    """
    ...
    def _initialize_game_state(self, board_bot: GameObject, board: Board) -> None:
        """Inisialisasi state game untuk turn saat ini"""
        self.bot = board_bot
        self.board = board
    ...
    def set_goal(self, goal: GameObject, force: bool):
        """
        Menetapkan goal berdasarkan jarak minimum dengan mempertimbangkan teleport
        Prioritas khusus untuk diamond dengan poin 2
        """
    ...
    def search_optimal(self, max_diamond: int, position: Position, total_points, total_distance) -> None:
        """
        Mencari rute optimal menggunakan algoritma rekursif dengan evaluasi kompetitif
        Mengurangi nilai diamond jika bot lain lebih dekat
        """
    ...
    def _check_early_returns(self) -> Optional[Tuple[int, int]]:
        """Mengecek kondisi untuk early return (inventori penuh atau waktu habis)"""
    ...
    def _handle_combat(self) -> Optional[Tuple[int, int]]:
        """Menangani combat dengan bot lain"""
        props = self.bot.properties
    ...
    def _evaluate_reset_buttons(self) -> None:
        """Mengevaluasi apakah perlu pergi ke reset button"""
    ...
    def _finalize_goal(self) -> None:
        """Finalisasi goal jika evaluasi terlalu rendah"""
        props = self.bot.properties
    ...
    def next_move(self, board_bot: GameObject, board: Board) -> Tuple[int]:
        """Main method untuk menentukan langkah selanjutnya"""
